...
export default function OrdersAreaChart({ allOrders }) {
    ...
    const chartData = Object.entries(ordersPerDay).map(([date, count]) => ({ date, orders: count }))

    return (
        <div className="w-full max-w-4xl h-[300px] text-xs">
            <h3 className="text-lg font-medium text-red-950 mb-4 pt-2 text-right"><span className="text-slate-500">Orders /</span> Day</h3>
