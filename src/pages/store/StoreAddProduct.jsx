...
export default function StoreAddProduct() {
    ...
    const [productInfo, setProductInfo] = useState({ name: '', description: '', mrp: 0, price: 0, category: '' })
            ...
            <div className="flex gap-3 mt-4">
                {Object.keys(images).map((key) => (
                    <label key={key} htmlFor={`images${key}`}>
                        <img className="h-15 w-auto border border-slate-200 rounded cursor-pointer" src={images[key] ? URL.createObjectURL(images[key]) : assets.upload_area} alt="" />
